...
bool encoder_update_kb(uint8_t index, bool clockwise)
{
    ...
    // 0 is left-half encoder, 1 is right-half encoder
    bool isLeftEncoder = index == 0;
    if (isLeftEncoder) 
    {
        ...
        else if(layer_state_is(LAYER_MOUSE))
        {
            // Move mouse vertically, etch a sketch style.
            if (clockwise)  tap_code(KC_MS_U);
            else            tap_code(KC_MS_D);
        }
        else
        {
            // Volume control.
    ...
    else
    {
        ...
        {
            // Move mouse horizontally, etch a sketch style.
            if (clockwise)  tap_code(KC_MS_R);
            else            tap_code(KC_MS_L);
        }
        else
        {
            // Navigate up and down by mouse scroll.
